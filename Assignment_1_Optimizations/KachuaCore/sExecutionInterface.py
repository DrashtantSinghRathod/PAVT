...
def handleSum(z3Vars, lhs, expr):
    if isinstance(expr.lexpr, kachuaAST.Var):
        left = expr.lexpr.__str__()
    if isinstance(expr.lexpr, kachuaAST.Num):
        left = expr.lexpr.__str__()

    if isinstance(expr.rexpr, kachuaAST.Var):
        right = expr.rexpr.__str__()
    if isinstance(expr.rexpr, kachuaAST.Num):
        right = expr.rexpr.__str__()
