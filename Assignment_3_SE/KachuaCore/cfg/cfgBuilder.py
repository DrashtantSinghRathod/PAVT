...
def buildCFG(ir, cfgName="", isSingle=False):
    ...
    for idx, item in enumerate(ir):
        #print(idx, item)
        if isinstance(item[0], kachuaAST.ConditionCommand) or isSingle:
            ...
            if idx + item[1] < len(ir) and (idx + item[1] not in leaderIndices) and (isinstance(item[0], kachuaAST.ConditionCommand)):
    ...
    # adding nodes to kachua graph
    cfg = kachuaCFG(cfgName)
    ...
    for node in cfg:
        ...
        if listSize:
            ...
            #print (irIdx, lastInstr, type(lastInstr), isinstance(lastInstr, kachuaAST.ConditionCommand))
            if isinstance(lastInstr, kachuaAST.ConditionCommand):
                if not isinstance(lastInstr.cond, kachuaAST.BoolFalse):
                    thenIdx = irIdx + 1 if (irIdx + 1 < len(ir)) else len(ir)
                    thenBB = indices2LeadersMap[thenIdx]
                    cfg.add_edge(node, thenBB, label='T')
                    
                if not isinstance(lastInstr.cond, kachuaAST.BoolTrue):
                    elseIdx = irIdx + ir[irIdx][1] if (irIdx + ir[irIdx][1] < len(ir)) else len(ir)
                    elseBB = indices2LeadersMap[elseIdx]
                    cfg.add_edge(node, elseBB, label='F')
            else:
                nextBB = indices2LeadersMap[irIdx + 1] if (irIdx + 1 < len(ir)) else endBB
                cfg.add_edge(node, nextBB)
